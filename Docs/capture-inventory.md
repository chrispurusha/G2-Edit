...
| QU-24, inputs 5/6 (zero-based 4/5) - A LOW SHELF, zero 69 Hz / pole 197 Hz (2026-09-25); inputs 19/20 are flat | 32 ch, 48000 Hz | every reverb `_stereo` capture, `g_medium_bright7`, `g_small_bright7`, and all of the older chorus/delay/oscillator/filter sets |
...
| **FltComb** | MEASURED 2026-09-12 on a linear frequency axis: delay 96000/f(Freq−9) − 1 samples (a major sixth below the dial, to 0.01 samples); g = (FB−64)/64; Notch a feed-forward comb, Peak and Deep mixed sections with fixed coefficient ratios and gains (§13.4); Level = mixer taper. Deep at |g| > 0.5 approximate. In the engine (§13). Captures (`fltcomb*.wav`) in the session scratchpad only, not kept. |
| **OscDual** | MEASURED 2026-09-12: levels linear; pulse duty (1 − PW/128)/2 from param 11 (tables swap 6/11); saw in phase at Phase 0, rotated Phase/128; sub-octave "through a fixed ~190 Hz shelf" - that was the capture chain (desk inputs 5/6, findings.md 2026-09-25); the engine follows the instrument's own part (reference §12.5). In the engine (§12). Mod depths and Sync not measured. Captures (`oscdual*.wav`) in the session scratchpad only, not kept. |
...
| **Noise** | MEASURED 2026-09-12 on desk inputs 5/6, whose low shelf bent the dark half (corners flattened near 129 Hz). The engine plays the instrument's own law since 2026-09-25 (reference §7.2a: 20 kHz -> 12 Hz, gain 1 + dial³/65536), which reproduces that capture through the shelf to a constant. |
| **OscC, OscD** | CHECKED 2026-09-12 against OscA, all six waveforms: level within 0.02 dB, harmonics within 0.25 dB. MEASURED 2026-09-17: OscC Saw, Sqr50/25/10 and Tri at 440-3520 Hz - the same two-sample edges as OscB, but a brighter triangle corner (reference §6.3). In the engine (shared oscillator, waveform read as a mode). FM not modelled. Captures in `G2Captures/oscb/`. |
| **OscB** | MEASURED 2026-09-17: all five waves at eight pitches 110 Hz-12.5 kHz, Sqr and DualSaw across Shape (0-127) at 440 Hz; every harmonic under 16 kHz within 0.1 dB of reference §6.3. The Sine set doubles as a chain calibration (high-pass: -4 dB at 110 Hz). Captures in `G2Captures/oscb/`. |
