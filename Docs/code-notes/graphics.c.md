...
## 18-21. moved to settingsPanels.c on 2026-09-16

The four settings-family panels (Synth Settings, Patch Settings, Performance Settings and Patch
Notes) and their drawing helpers left this file so that the PLUG-IN could have them: graphics.c is
the application's render loop and is not in do-plugin's source list, so in G2 Alike each of these
opened and was never drawn. The four sections are now §1-§4 of Docs/code-notes/settingsPanels.c.md,
verbatim.

These numbers are left as a GAP rather than closed up, so that every other `// notes §k` marker in
graphics.c still resolves to the section it always did.
...
## 23-33. moved to floatingPanels.c on 2026-09-16

The floating-panel coordinator - the one list of the panels, the sort/raise/hit-test walks, and the
four callbacks each host registers into SynthLib's popup ordering - left this file for the same
reason, and on the same day. The eleven sections are now §1-§11 of
Docs/code-notes/floatingPanels.c.md, verbatim. A gap again, for the reason given above.
